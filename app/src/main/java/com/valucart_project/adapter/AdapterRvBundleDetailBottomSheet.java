...
public class AdapterRvBundleDetailBottomSheet extends RecyclerView.Adapter<AdapterRvBundleDetailBottomSheet.GroceryViewHolder>{

    private ArrayList<BundleDetail.Data.Products.ProductsAlternatives> horizontalGrocderyList;
    Context context;
    GroceryViewHolder holderGrocery;
    OnItemSelection  onItemSelection;
    Library library;

    public AdapterRvBundleDetailBottomSheet( ArrayList<BundleDetail.Data.Products.ProductsAlternatives> horizontalGrocderyList1, Context context, OnItemSelection  onItemSelection1){
    ...
    public GroceryViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View groceryProductView = LayoutInflater.from(parent.getContext()).inflate(R.layout.bundle_bottom_sheet_detail_item, parent, false);
    ...
    public void onBindViewHolder(final GroceryViewHolder holder, final int position) {
        ...
        holder.tvProductSelect.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                onItemSelection.onItemSelected("bottomsheet",position);
            }
        });
        ...
        //Picasso.with(context).load(horizontalGrocderyList.get(position).getData().getThumbnail() + "?w=151".replaceAll(" ", "%20")).into(holder.ivProductImage);
        library.displayImage(horizontalGrocderyList.get(position).getData().getThumbnail() + "?w=151",holder.ivProductImage);

        holder.tvProductName.setText(horizontalGrocderyList.get(position).getData().getName());
        holder.tvProductOldPrice.setPaintFlags(holder.tvProductOldPrice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
        holder.tvWeight.setText(""+horizontalGrocderyList.get(position).getData().getPackaging_quantity()+horizontalGrocderyList.get(position).getData().getPackaging_quantity_unit().getSymbol());
        holder.tvProductOldPrice.setText(""+horizontalGrocderyList.get(position).getData().getMaximum_selling_price()+" AED");
        holder.tvProductOldPrice.setText(""+horizontalGrocderyList.get(position).getData().getMaximum_selling_price()+" AED");
        holder.tvProductPrice.setText(""+horizontalGrocderyList.get(position).getData().getValucart_price()+" AED");
    }
    ...
    public class GroceryViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView tvProductName,tvProductOldPrice,tvProductSelect,tvWeight,tvProductPrice;
        LinearLayout llItemHeader,llProduct;
        ...
        public GroceryViewHolder(View view) {
            ...
            tvProductSelect = view.findViewById(R.id.tvProductSelect);
